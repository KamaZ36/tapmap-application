...
from app.domain.value_objects.point import Point, Coordinates
...
@pytest.fixture
def point_inside():
    return Point(address="Тест", coordinates=Coordinates(latitude=55.7, longitude=37.5))

@pytest.fixture
def point_outside():
    return Point(address="Тест", coordinates=Coordinates(latitude=56.0, longitude=38.0))
...
class TestPointInside:
    ...
    def test_point_on_boundary(self, moscow_city: City):
        boundary_point = Point(address="ТЕСТ", coordinates=Coordinates(latitude=55.5, longitude=37.5))
...
class TestRouteInside:
    ...
    def test_route_inside(self, moscow_city: City):
        points = [
            Point(address="Тест1", coordinates=Coordinates(latitude=55.6, longitude=37.4)),
            Point(address="Тест2", coordinates=Coordinates(latitude=55.7, longitude=37.5)),
            Point(address="Тест3", coordinates=Coordinates(latitude=55.8, longitude=37.6))
        ]
        assert moscow_city.is_route_inside(points) is True

    def test_route_partially_outside(self, moscow_city, point_outside):
        points = [
            Point(address="Тест", coordinates=Coordinates(latitude=55.7, longitude=37.5)),
            point_outside
        ]
