...
from app.domain.exceptions.order import InvalidOrderStatusTransition, OrderCannotTwoPoints
    ...
    def test_order_creation_with_less_than_two_points_error(self):
        with pytest.raises(OrderCannotTwoPoints):
            Order(
                customer_id=uuid7(),
                city_id=uuid7(),
                points=[Point(address="Тест", coordinates=Coordinates(latitude=51.23, longitude=41.23))],
                price=Money(1000),
                service_commission=Money(100),
                travel_distance=1000,
                travel_time=10
...
class TestEdgeCases:
    ...
    def test_assign_driver_in_invalid_status_fails(self, sample_order):
        ...
        for status in OrderStatus:
            if status != OrderStatus.driver_search:
                sample_order.status = status
                with pytest.raises(InvalidOrderStatusTransition):
                    sample_order.assing_driver(uuid7(), 1000, 10)