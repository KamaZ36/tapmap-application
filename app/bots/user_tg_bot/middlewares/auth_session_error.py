...
from app.bots.user_tg_bot.messages.errors import AtuhSessionNotFoundErrorMessage
from app.bots.user_tg_bot.states.user import RegisterUserStates
...
class AuthSessionMiddleware(BaseMiddleware):
    async def __call__(
        ...
        if auth_session is None:
            await self._handle_auth_error(event=event, data=data, e=e)
            return
    ...
    async def _handle_auth_error(
        self, event: TelegramObject, data: dict[str, Any], e: Exception
    ):
        ...
        await context.set_state(RegisterUserStates.get_phone_number)
        return
