...
from app.presentation.api.v1.schemas.driver import CreateDriverSchema, ResponseDriverSchema, UpdateDriverLocationSchema
from app.presentation.api.v1.schemas.location import CoordinatesSchema
from app.presentation.api.v1.schemas.order import GetOrderSFiltersSchema, ResponseExtendedOrderSchema, ResponseOrderSchema
...
@router.post(
    '',
    summary="Создать водителя (Только администраторам)"
)
...
@router.get(
    '/me/orders/active',
    summary="Получить активный заказ текущего водителя"
)
...
async def get_driver_vehicles(
    current_user: CurrentUserDep,
    interactor: FromDishka[GetVehiclesInteraction]
) -> list[ResponseVehicleSchema]:
    vehicles = await interactor(driver_id=current_user.user_id, current_user=current_user)
    return [ResponseVehicleSchema.from_domain(vehicle) for vehicle in vehicles]

@router.get(
    '/me/orders',
    summary="Получить список заказов текущего водителя"
)
async def get_driver_order_list(
    ...
    filters = GetOrderSFiltersSchema(driver_id=current_user.user_id, offset=offset, limit=limit)
...
@router.patch(
    '/me/on_shift',
    summary="Переключить статус смены текущего водителя"
)
async def switch_on_shift_status(
    ...
    await interactor(current_user=current_user, driver_id=current_user.user_id, status=status)
    return JSONResponse(status_code=200, content={'message': 'success'})

@router.patch(
    '/me/location',
    summary="Обновить последнюю геолокацию текущего водителя"
)
...
async def get_location_driver(
    ...
    return CoordinatesSchema.model_validate(asdict(coordinates)) if coordinates else None
