...
from app.application.commands.order import AddPointToDraftOrderCommand, CreateDraftOrderCommand, AddCommentToDraftOrderCommand
...
from app.presentation.api.v1.schemas.draft_order import AddCommentSchema, AddPointToDraftOrderSchema, CreateDraftOrderSchema, ResponseDraftOrderSchema
...
@router.post(
    ...
    responses={
        status.HTTP_201_CREATED: {'model': ResponseDraftOrderSchema}
    }
)
...
@router.get(
    '',
    summary="Получить черновик заказа текущего пользователя"
)
async def get_draft_order(
    current_user: CurrentUserDep,
    interactor: FromDishka[GetDraftOrderInteraction]
) -> ResponseDraftOrderSchema:
...
@router.delete(
    '',
    summary="Удалить черновик заказа текущего пользователя"
)
async def delete_draft_order(
    current_user: CurrentUserDep,
    interactor: FromDishka[DeleteDraftOrderInteraction]
) -> None:
