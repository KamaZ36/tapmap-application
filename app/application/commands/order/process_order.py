...
class ProcessOrderInteraction:
    ...
    async def __call__(self, order_id: UUID) -> bool:
        ...
        feeding_distance = await self.route_service.get_distance_route(
            [driver.last_location, order.points[0].coordinates]
        )
        feeding_time = await self.route_service.get_time_route(feeding_distance)

        order.assign_driver(
            driver_id=driver.id,
            feeding_distance=feeding_distance,
