...
class UpdateStatusInteraction:
    ...
    async def __call__(self, current_user: CurrentUser, command: UpdateOrderStatusCommand) -> OrderStatus:
        ...
        logger.info(f"Пользователь {current_user.user_id} обновил статус заказа {order.id} с {prev_status.value} на {order.status.value}")
        ...
        await self.message_broker.publish(topic=settings.order_status_update_topic, key=str(order.id), value=event)
        return order.status
    ...
    def _validate_permissions(self, current_user: CurrentUser, order: Order) -> None:
        if order.driver_id == current_user.user_id and UserRole.driver in current_user.roles:
            return
