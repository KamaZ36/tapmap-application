...
class UpdateLocationInteraction:
    ...
    async def __call__(
        self, command: SetBaseUserLocationCommand, current_user: CurrentUser
    ) -> None:
        driver = await self.driver_repository.try_get_by_id(current_user.user_id)

        coordinates = Coordinates(
            latitude=command.coordinates[0], longitude=command.coordinates[1]
        )
